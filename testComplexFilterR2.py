...
train_file = "dataset/Filter/sallen_keyBP_R2_v1.csv"
...
X_cols = [c for c in df.columns if c.startswith('V')]
Y_col  = 'R2'

X = df[X_cols].apply(lambda col: col.map(str_to_complex)).values
Y = pd.to_numeric(df[Y_col]).values.astype(float).reshape(-1, 1)

# ── Normalizzazione input complessi ───────────────────────────────────────────
X_real = X.real
X_imag = X.imag
mean_r = X_real.mean(axis=0);  std_r = X_real.std(axis=0) + 1e-8
mean_i = X_imag.mean(axis=0);  std_i = X_imag.std(axis=0) + 1e-8
X = ((X_real - mean_r) / std_r) + 1j * ((X_imag - mean_i) / std_i)
...
# ── StandardScaler sul target ─────────────────────────────────────────────────
scaler = StandardScaler()
Y_train_n = scaler.fit_transform(Y_train)

print("Normalizzazione target (StandardScaler):")
print(f"  R2: mean={scaler.mean_[0]:.4e}  std={scaler.scale_[0]:.4e}")

n_inputs  = X.shape[1]
n_targets = 1

ARCH      = [n_inputs, 8, n_targets]
EPOCHS    = 1000
THRESHOLD = 1e-6
...
for use_qr in [False, True]:
    ...
    # ── FIX: de-normalizzare le predizioni prima di calcolare le metriche ────
    Y_pred_n = np.array([net.feedforward(list(x)) for x in X_test]).reshape(-1, 1)
    Y_pred   = scaler.inverse_transform(Y_pred_n.real)   # ← .real scarta rumore immaginario, poi de-normalizza
    # ─────────────────────────────────────────────────────────────────────────
...
# ── 3. Predicted vs True (spazio originale) ───────────────────────────────
